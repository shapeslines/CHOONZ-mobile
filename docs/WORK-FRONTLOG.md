...
|---:|---|---|---|
| 1 | **Mobile lockfile re-lock** — `package-lock.json` regenerated under a fresh dependency review; the `@react-native/*` `0.86.2` skew under `react-native@0.86.3` is closed, `package.json` unchanged, audit flat at 13 moderate / 0 high / 0 critical, `image-size` still absent. Two recorded causes were corrected: `npm ci` **does** reify inside the checkout (the earlier proof dropped the tracked `.npmrc`), and red hosted CI is a runner/billing failure, not the install step | [security/lockfile-relock-2026-09.md](security/lockfile-relock-2026-09.md); [security/npm-audit-exception.md](security/npm-audit-exception.md) §"Record — 2026-09-04"; fence `package-lock.json`, `docs/security/**` | **PR open — owner merge** on lane `lane/choonzm-lockfile/20260904` |
...
| 7 | **`series-engine`** — CHOONZ #142 (M5b) froze one engine per series: `SeriesCreate.engine` (same closed enum, same `422`), `SeriesRead.engine` derived from the newest bout and defaulted to `ah-scripted` for pre-M5b series. Client landed the contract — `Series` / `SeriesCreateInput` types, `decodeSeries` with the explicit absent-key default, `createSeries` forwarding `engine?`, and a fixture series whose first bout carries the engine the series reads back from. **No series screen exists in this app**, so nothing renders it yet | [plans/fightv2-ui.md](plans/fightv2-ui.md) §"Series follow-up (M5b)"; fence `src/lib/{types,decoder,api,fixture-match-service}.ts`, `tests/{fight-api,fixture-match-service}.test.ts` | **done** on lane `lane/choonzm-series-engine/20260904` |
| 8 | **Mechanics-lab revision pin** — CHOONZ M5 flipped `ENGINE_REVISION` to `"2"`, so the lab routes now default to the `scenarios.v2.json` corpus; the client sends no `engine_revision` and silently follows. Decide whether the lab pins `"1"`, offers a selector, or keeps following the server default, and state the revision in the header either way | CHOONZ `docs/mobile-integration.md` §"Engine revision 2 (M5)"; fence `src/lib/{api,types}.ts`, `src/app/lab.tsx`, `tests/lab-screen.test.tsx` | **open** — gate met (M5 merged) |
| 9 | M1 identity (`clubheavy.choonz`), EAS profiles, icons | `app.json`, `eas.json` | **blocked — owner M1** |
| 10 | M2/M3 Apple + Google providers, sign-in scope | Supabase console + `src/providers/auth-*` | **blocked — owner M2/M3** |
| 11 | M-S4 IAP (RevenueCat) | `docs/store-readiness.md` §4 | **blocked — owner M4 / backend P-S4** |

## Done (recent)

| Work | Evidence |
|---|---|
| Series engine contract — `Series` types, `decodeSeries`, `createSeries`, fixture series | lane `lane/choonzm-series-engine/20260904` |
