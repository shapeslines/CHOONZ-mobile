...
| 8 | **Mechanics-lab revision pin** — CHOONZ M5 flipped `ENGINE_REVISION` to `"2"`, so the lab routes default to the `scenarios.v2.json` corpus. Owner decision: a `LAB / ENGINE REVISION` selector — `SERVER` (default, sends no pin and follows the backend), `1`, `2` — threaded through list, detail and replay; the header keeps stating the revision the server *declared* | [plans/lab-revision.md](plans/lab-revision.md); fence `src/lib/{api,types}.ts`, `src/app/lab.tsx`, `tests/{api.test.ts,lab-screen.test.tsx}` | **done** on lane `lane/choonzm-lab-revision/20260904` |
...
| Work | Evidence |
|---|---|
| Mechanics-lab engine revision selector (`SERVER` / `1` / `2`) | lane `lane/choonzm-lab-revision/20260904` |
