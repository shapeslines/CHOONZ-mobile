...
| **Mechanics-lab `engine_revision` default flip (CHOONZ M5) → lab selector** | Now | [lab-revision.md](lab-revision.md) | `src/lib/{api,types}.ts`, `src/app/lab.tsx`, `tests/{api.test.ts,lab-screen.test.tsx}` | none (M5 merged) | **done — rank 8** (`SERVER` default sends no pin; the header still states the revision the server declared) |
